...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5>Ratings: ${product.rating.rate} (${product.rating.count})</h5>
...
const showBuyText = () => {
  ...
  if (total == 0) {
      ...
      span.innerHTML = `<h2 class="my-5 text-center" style="color: red">Please select Item's to buy!!</h2>`;
      text.appendChild(span);
  } else {
    const span = document.createElement('span');
    span.innerHTML = `<h2 class="my-5 text-center" style="color: green">Your order has been placed.</h2>`;
    text.appendChild(span);
  //   total.innerText = '0';
  //   price.textContent = '0';
